    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full">
      {projects.map((project, idx) => (
        <motion.div
          ...
          className="group cursor-pointer border-r border-b border-foreground/10 bg-background flex flex-col hover:bg-foreground/[0.02] transition-colors duration-500"
        >
          <div className="relative w-full aspect-[4/3] overflow-hidden border-b border-foreground/10">
            <Image
              src={project.image}
              alt={project.title}
              fill
            ...
            <div className="absolute inset-0 bg-background/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center backdrop-blur-[2px]">
              <span className="text-xs font-mono uppercase tracking-widest bg-background/80 px-4 py-2 border border-foreground/20 flex items-center gap-2">
          ...
          <div className="p-6 md:p-8 flex flex-col flex-grow justify-between gap-6">
            <div>
              ...
              <p className="text-foreground/50 text-sm font-light line-clamp-2 leading-relaxed">
                {project.description}
              </p>
            </div>
            ...
            {project.techStack && project.techStack.length > 0 && (
              <div className="flex flex-wrap gap-x-2 gap-y-1 mt-auto pt-4">
                {project.techStack.slice(0, 3).map((tech, index) => (
                  <span
                    key={tech}
                    className="text-[10px] font-mono uppercase tracking-widest text-foreground/40 flex items-center gap-2"
                  >
                    {tech}{" "}
